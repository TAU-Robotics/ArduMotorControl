...
void motorComand(float cntrlValue){
  ...
  if (cntrlValue >= 0){
    analogWrite(PWM_PIN,int(cntrlValue * 255));
...
float ReadRPM(){
    ...
    // float rpmEncoder = rotationDir * 60 * (1000000 / (float(encoderDTmicros) * ENCODER_PPR) / GEAR_RATIO); // dt in microseconds with 3 pulses per revolution
...
float ReadRadS(){
    ...
    // float rpmEncoder = rotationDir * (1000000 / (float(encoderDTmicros) * ENCODER_PPR) / GEAR_RATIO * TWO_PI); // dt in microseconds with 3 pulses per revolution
